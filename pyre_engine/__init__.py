...
pytest, unchanged. The Azure SDK is imported lazily, and only by the modules
that talk to Blob or Redis.

    config.py      app settings + config/sources.yaml, and problems() over both
    ...
    sinks.py       where signals and alerts go - one destination per stream
    records.py     the signal and alert schemas, defined once
